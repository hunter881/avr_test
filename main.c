...
#if F_CPU != 8000000UL
#error "This SK6812 bit-bang timing is tuned for F_CPU = 8MHz only."
#endif
...
{
    // Cycle-tuned software waveform generator for 800kHz SK6812 protocol at 8MHz CPU.
    // 0-bit high phase: ~0.375us (3 cycles)
    // 1-bit high phase: ~0.75us  (6 cycles)
    // bit period:       ~1.25us (10 cycles typical, last bit can be ~1.125us)
    uint8_t cnt = 8;
    uint8_t hi = LED_PORT | (1 << LED_PIN);
    uint8_t lo = LED_PORT & (uint8_t)~(1 << LED_PIN);
    ...
    __asm__ __volatile__(
        ...
        "dec %[cnt]\n\t"
        "brne 1b\n\t"
        : [byte] "+r" (byte), [cnt] "+r" (cnt)
        : [port] "I" (_SFR_IO_ADDR(LED_PORT)), [hi] "r" (hi), [lo] "r" (lo)
        : "memory"
    );
}
...
static void sk6812_show(const rgbw_t *leds, uint8_t count)
{
    ...
    // Reset/latch time per datasheet note: >200us low
    _delay_us(300);
}
